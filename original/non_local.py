...
def _convND(ip, rank, channels):
    ...
    if rank == 3:
        x = Conv1D(channels, 1, padding='same', use_bias=False, kernel_initializer='he_uniform')(ip)
    elif rank == 4:
        x = Conv2D(channels, (1, 1), padding='same', use_bias=False, kernel_initializer='he_uniform')(ip)
    else:
        x = Conv3D(channels, (1, 1, 1), padding='same', use_bias=False, kernel_initializer='he_uniform')(ip)
    return x

if __name__ == '__main__':
    a = Input(shape=(4, 4, 3))
    b = non_local_block(a, compression=1, mode='embedded')
    model = Model(inputs=a, outputs=b)
    model.summary()

    img = np.load("train_images.npy")
    x = model.predict(img[:1, 0, :4, :4, :])
    print(x.shape)
    # print(img[:1, :2, :4, :4, :], "\n-------\n")
    print(x)
